 ...
 *       {@link android.provider.Settings#ACTION_REQUEST_SET_AUTOFILL_SERVICE Settings#ACTION_REQUEST_SET_AUTOFILL_SERVICE} intent can be used to launch such
 *       Settings screen).
 * </ol>
 *
 * <a name="BasicUsage"></a>
 ...
 * <ol>
 *   <li>User focus an editable {@link android.view.View View}.
 *   <li>View calls {@link android.view.autofill.AutofillManager#notifyViewEntered(android.view.View) AutofillManager#notifyViewEntered(android.view.View)}.
 *   <li>A {@link android.view.ViewStructure ViewStructure} representing all views in the screen is created.
 ...
 *       {@link #onFillRequest(android.service.autofill.FillRequest,android.os.CancellationSignal,android.service.autofill.FillCallback)}.
 *   <li>The service replies through {@link android.service.autofill.FillCallback#onSuccess(android.service.autofill.FillResponse) FillCallback#onSuccess(FillResponse)}.
 ...
 *
 * <p>Typically, the
 * {@link #onFillRequest(android.service.autofill.FillRequest,android.os.CancellationSignal,android.service.autofill.FillCallback)} will:
 * <ol>
 ...
 *   <li>Create a {@link android.service.autofill.Dataset Dataset} for each set of user's data that match those fields.
 *   <li>Fill the dataset(s) with the proper {@link android.view.autofill.AutofillId AutofillId}s and {@link android.view.autofill.AutofillValue AutofillValue}s.
 *   <li>Add the dataset(s) to the {@link android.service.autofill.FillResponse FillResponse} passed to
 ...
 * {@code null} to {@link android.service.autofill.FillCallback#onSuccess(android.service.autofill.FillResponse) FillCallback#onSuccess(FillResponse)}; if the service encountered an error
 * processing the request, it should call {@link android.service.autofill.FillCallback#onFailure(java.lang.CharSequence) FillCallback#onFailure(CharSequence)}. For
 * performance reasons, it's paramount that the service calls either
 * {@link android.service.autofill.FillCallback#onSuccess(android.service.autofill.FillResponse) FillCallback#onSuccess(FillResponse)} or {@link android.service.autofill.FillCallback#onFailure(java.lang.CharSequence) FillCallback#onFailure(CharSequence)} for
 * each {@link #onFillRequest(android.service.autofill.FillRequest,android.os.CancellationSignal,android.service.autofill.FillCallback)} received - if it
 ...
 * {@link android.service.autofill.SaveInfo SaveInfo} object in the {@link android.service.autofill.FillResponse FillResponse}. See {@link android.service.autofill.SaveInfo SaveInfo} for more details and
 * examples.
 *
 ...
 *       authentication) - see
 * {@link android.service.autofill.FillResponse.Builder#setAuthentication(android.view.autofill.AutofillId[],android.content.IntentSender,android.widget.RemoteViews) FillResponse.Builder#setAuthentication(AutofillId[], android.content.IntentSender, android.widget.RemoteViews)}.
 *   <li>To unlock a specific dataset (for example, by providing a CVC for a credit card) - see
 *       {@link android.service.autofill.Dataset.Builder#setAuthentication(android.content.IntentSender) Dataset.Builder#setAuthentication(android.content.IntentSender)}.
 * </ul>
 *
 ...
 *
 * <ol>
 *   <li>The first {@link android.service.autofill.FillResponse FillResponse} contains datasets with the credentials for the financial
 ...
 *       banking apps.
 *   <li>When the user select a banking app, the service replies with a new {@link android.service.autofill.FillResponse FillResponse}
 ...
 * (see {@link android.service.autofill.Dataset.Builder#setAuthentication(android.content.IntentSender) Dataset.Builder#setAuthentication(android.content.IntentSender)}), and to include
 ...
 * {@link android.service.autofill.FillResponse FillResponse} with just the datasets for the partition that originated the request (i.e.,
 ...
 * {@link #onFillRequest(android.service.autofill.FillRequest,android.os.CancellationSignal,android.service.autofill.FillCallback)} call for that partition,
 * and so on.
 *
 ...
 * {@link #onFillRequest(android.service.autofill.FillRequest,android.os.CancellationSignal,android.service.autofill.FillCallback)} call, with the values of
 * the fields {@code id1} and {@code id2} prepopulated, so the service could then fetch the address
 * for the Flanders account and return the following {@link android.service.autofill.FillResponse FillResponse} for the address partition:
 *
 * <pre class="prettyprint">
 * new FillResponse.Builder()
 ...
 * </pre>
 *
 * <p>When the service returns multiple {@link android.service.autofill.FillResponse FillResponse}, the last one overrides the previous;
 * that's why the {@link android.service.autofill.SaveInfo SaveInfo} in the 2nd request above has the info for both partitions.
 *
 ...
 * calling {@link android.service.autofill.FillResponse.Builder#setIgnoredIds(android.view.autofill.AutofillId...) FillResponse.Builder#setIgnoredIds(AutofillId...)} so the system does not trigger
 * a new {@link #onFillRequest(android.service.autofill.FillRequest,android.os.CancellationSignal,android.service.autofill.FillCallback)} when these views are
 * focused.
 *
 ...
 *       {@link android.service.autofill.Dataset.Builder#setAuthentication(android.content.IntentSender) Dataset.Builder#setAuthentication(android.content.IntentSender)} to
 *       unlock.
 *   <li>Include the web domain in the custom presentation for the
 *       {@link android.service.autofill.Dataset.Builder#setValue(android.view.autofill.AutofillId,android.view.autofill.AutofillValue,android.widget.RemoteViews) Dataset.Builder#setValue(AutofillId, AutofillValue, android.widget.RemoteViews)}.
 ...
 *   <li>Similarly, when adding a {@link android.service.autofill.SaveInfo SaveInfo} object for the request, the service should
 *       include the above disclaimer in the {@link android.service.autofill.SaveInfo.Builder#setDescription(java.lang.CharSequence) SaveInfo.Builder#setDescription(CharSequence)}.
 * </ol>
 *
 ...
 * <ol>
 * <li>In the first
 * {@link #onFillRequest(android.service.autofill.FillRequest,android.os.CancellationSignal,android.service.autofill.FillCallback) fill request}, the service
 * adds a {@link android.service.autofill.FillResponse.Builder#setClientState(android.os.Bundle) FillResponse.Builder#setClientState(android.os.Bundle)} in
 * the response, containing the autofill ids of the partial fields present in the screen.
 * <li>In the second
 * {@link #onFillRequest(android.service.autofill.FillRequest,android.os.CancellationSignal,android.service.autofill.FillCallback) fill request}, the service
 * retrieves the {@link android.service.autofill.FillRequest#getClientState() FillRequest#getClientState()}, gets the autofill ids
 * set in the previous request from the client state, and adds these ids and the
 * {@link android.service.autofill.SaveInfo#FLAG_SAVE_ON_ALL_VIEWS_INVISIBLE SaveInfo#FLAG_SAVE_ON_ALL_VIEWS_INVISIBLE} to the {@link android.service.autofill.SaveInfo SaveInfo} used in the second
 * response.
 * <li>In the {@link #onSaveRequest(android.service.autofill.SaveRequest,android.service.autofill.SaveCallback) save request}, the service uses the
 * proper {@link android.service.autofill.FillContext FillContext} to get the value of each field (there is one fill
 ...
 * <h3>Privacy</h3>
 *
 * <p>The {@link #onFillRequest(android.service.autofill.FillRequest,android.os.CancellationSignal,android.service.autofill.FillCallback)} method is called
 ...
 *
 * <ol>
 *   <li>Set the user data through {@link android.view.autofill.AutofillManager#setUserData(android.service.autofill.UserData) AutofillManager#setUserData(UserData)}. This data is
 ...
 *   {@link android.service.autofill.FillResponse.Builder#setFieldClassificationIds(android.view.autofill.AutofillId...) FillResponse.Builder#setFieldClassificationIds(AutofillId...)}.
 *   <li>Verify the results through {@link android.service.autofill.FillEventHistory.Event#getFieldsClassification() FillEventHistory.Event#getFieldsClassification()}.
 *   <li>Use the results to dynamically create {@link android.service.autofill.Dataset Dataset} or {@link android.service.autofill.SaveInfo SaveInfo} objects in
 *   subsequent requests.
 * </ol>
 *
 ...
 * <ul>
 * <li>No manual autofill requests. Hence, the {@link android.service.autofill.FillRequest FillRequest}
 * {@link android.service.autofill.FillRequest#getFlags() FillRequest#getFlags()} never have the {@link android.service.autofill.FillRequest#FLAG_MANUAL_REQUEST FillRequest#FLAG_MANUAL_REQUEST} flag.
 ...
 * data with invalid input. For example, when you parse the {@link android.service.autofill.FillRequest FillRequest} and detect a
 * password field, you could check if its
 * {@link android.app.assist.AssistStructure.ViewNode#getInputType()
 * input type} has password flags and if so, don't add it to the {@link android.service.autofill.SaveInfo SaveInfo} object.
 * <li>The autofill context is not always {@link android.view.autofill.AutofillManager#commit() AutofillManager#commit()} when an HTML
 * form is submitted. Hence, you must use other mechanisms to trigger save, such as setting the
 * {@link android.service.autofill.SaveInfo#FLAG_SAVE_ON_ALL_VIEWS_INVISIBLE SaveInfo#FLAG_SAVE_ON_ALL_VIEWS_INVISIBLE} flag on {@link android.service.autofill.SaveInfo.Builder#setFlags(int) SaveInfo.Builder#setFlags(int)}
 * or using {@link android.service.autofill.SaveInfo.Builder#setTriggerId(android.view.autofill.AutofillId) SaveInfo.Builder#setTriggerId(AutofillId)}.
 ...
 *
 * <p>Service must call one of the {@link android.service.autofill.FillCallback FillCallback} methods (like
 * {@link android.service.autofill.FillCallback#onSuccess(android.service.autofill.FillResponse) FillCallback#onSuccess(FillResponse)}
 * or {@link android.service.autofill.FillCallback#onFailure(java.lang.CharSequence) FillCallback#onFailure(CharSequence)})
 * to notify the result of the request.
 *
 * @param request the {@link android.service.autofill.FillRequest FillRequest} to handle.
 *        See {@link android.service.autofill.FillResponse FillResponse} for examples of multiple-sections requests.
...
public abstract void onFillRequest(@androidx.annotation.NonNull android.service.autofill.FillRequest request, @androidx.annotation.NonNull android.os.CancellationSignal cancellationSignal, @androidx.annotation.NonNull android.service.autofill.FillCallback callback);
 ...
 * down&mdash;the service could keep the {@link android.service.autofill.SaveRequest SaveRequest} and reuse it later,
 * but the service <b>must always</b> call {@link android.service.autofill.SaveCallback#onSuccess() SaveCallback#onSuccess()} or
 * {@link android.service.autofill.SaveCallback#onSuccess(android.content.IntentSender) SaveCallback#onSuccess(android.content.IntentSender)} right away.
 *
 ...
 *
 * @param request the {@link android.service.autofill.SaveRequest SaveRequest} to handle.
 *        See {@link android.service.autofill.FillResponse FillResponse} for examples of multiple-sections requests.
 * This value must never be {@code null}.
 * @param callback object used to notify the result of the request.
...
public abstract void onSaveRequest(@androidx.annotation.NonNull android.service.autofill.SaveRequest request, @androidx.annotation.NonNull android.service.autofill.SaveCallback callback);

/**
 * Called when the Android system disconnects from the service.
 *
 * <p> At this point this service may no longer be an active {@link android.service.autofill.AutofillService AutofillService}.
 * It should not make calls on {@link android.view.autofill.AutofillManager AutofillManager} that requires the caller to be
 * the current service.
 * @apiSince 26
 */
 ...
 * replies to a {@link #onFillRequest(android.service.autofill.FillRequest,android.os.CancellationSignal,android.service.autofill.FillCallback)} by calling
 * {@link android.service.autofill.FillCallback#onSuccess(android.service.autofill.FillResponse) FillCallback#onSuccess(FillResponse)} or {@link android.service.autofill.FillCallback#onFailure(java.lang.CharSequence) FillCallback#onFailure(CharSequence)}
 ...
 * finishing the {@link android.service.autofill.FillCallback FillCallback}.
 *
 * @return The history or {@code null} if there are no events.
 *
 * @throws java.lang.RuntimeException if the event history could not be retrieved.
 * @apiSince 26
 */
